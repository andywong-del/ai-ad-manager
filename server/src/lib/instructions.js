...
|---|---|---|
| **ANALYZE** | "check performance", "ROAS", "spend", "insights", "report", "audit", "how are my", "what's working", "CPL", "CPA", "CTR", "點樣", "最近點", "點解咁貴", "有咩要熄", "邊個好", "加錢", analytics question | Call exactly 2 tools in parallel as your FIRST action — no text, no clarifying question: (1) analyze_performance() (2) load_skill("insights-reporting"). analyze_performance() makes a single Meta API call and returns { current_7d, previous_7d, baseline_30d, _benchmarks } — all 3 periods pre-split with optimization_goal pre-joined in each campaign row. _benchmarks[goal].avg_cost_per_result is the 30-day account baseline. Use benchmarks with CTR/CPM/frequency trends to classify each campaign into one of 5 diagnostic statuses: 🚨 預算流失警告 (spend>0, results=0), ⚠️ 創意吸引力衰退 (CPA↑+CTR↓+freq>2.5), ⚔️ 流量競爭加劇 (CPA↑+CTR stable+CPM↑), ⚖️ 表現穩定運行 (CPA ±20%), 🚀 爆發增長模式 (CPA<-20%+CTR stable) — see insights-reporting skill Step 2 for full decision tree. Then follow insights-reporting skill which auto-routes to Scenario A/B/C/D and outputs dual-stream (Chat strategic briefing + Canvas formal report). |
